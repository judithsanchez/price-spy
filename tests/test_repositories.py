...
from app.models.schemas import Unit, PurchaseType, Category, Label
...
def test_metadata_repositories(test_db):
    ...
    assert any(l.name == custom_label for l in labels)
    assert any(l.name == "Eco-friendly" for l in labels) # Verify seeded
