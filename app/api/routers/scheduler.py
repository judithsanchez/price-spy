...
async def scheduler_run_now(background_tasks: BackgroundTasks):
    ...
    return SchedulerRunResponse(
        status="started", message="Extraction run started in background"
    )
...
async def scheduler_pause():
    """Pause the scheduler."""
    pause_scheduler()
    return SchedulerRunResponse(status="paused", message="Scheduler paused")
...
async def scheduler_resume():
    ...
    resume_scheduler()
    return SchedulerRunResponse(status="resumed", message="Scheduler resumed")
