...
async def get_products_summary(db=Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "active_categories": len(set(p.category for p in all_products if p.category))
        }
    finally:
        db.close()

@router.patch("/{product_id}", response_model=ProductResponse)
async def patch_product(product_id: int, product_update: ProductUpdate, db=Depends(get_db)):
...
async def merge_products(request: MergeRequest, db=Depends(get_db)):
    ...
    try:
        ...
        return {"status": "success", "message": f"Product {request.source_id} merged into {request.target_id}"}
